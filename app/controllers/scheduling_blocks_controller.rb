class SchedulingBlocksController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @scheduling_block.save
        ...
        format.json { render action: 'show', status: :created, location: @scheduling_block }
      else
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to scheduling_blocks_url }
      format.json { head :no_content }
    end
  end
  ...
  private
    ...
    def scheduling_block_params
      params.require(:scheduling_block).permit(:ini_bloq, :fin_bloq)
    end
end
